...
#[cfg(all(
    feature = "constantine_compatible",
    not(feature = "gnark_crypto_compatible")
))]
...
pub fn MapToCurve2(u: Fq2) -> G2Affine {
    ...
    let c1 = Fq2 {
        ...
        .unwrap(),
        c1: Fq::from_str(
            "266929791119991161246907387137283842545076965332900288569378510910307636689",
        )
        .unwrap(),
    };
    ...
    let c3 = Fq2 {
        ...
        .unwrap(),
        c1: Fq::from_str(
            "15403170217607925661891511707918230497750592932893890913125906786266381721360",
        )
        .unwrap(),
    };
    ...
    let c4 = Fq2 {
        ...
        .unwrap(),
        c1: Fq::from_str(
            "355906388159988214995876516183045123393435953777200384759171347880410182252",
        )
        .unwrap(),
    };
    ...
    x = if gx2.legendre().is_qr() && !gx1.legendre().is_qr() {
        ...
        x
    }; //    26.   x = CMOV(x, x2, e2)    # x = x2 if gx2 is square and gx1 is not
    ...
    if signsNotEqual == 0 {
        ...
        y = tv1
    }; //    33.   y = CMOV(-y, y, e3)       # Select correct sign of y
...
pub fn MapToCurve2(u: Fq2) -> G2Affine {
    ...
    let z = Fq2 {
        ...
        .unwrap()
            * Fq::from(Fq::R).inverse().unwrap(),
        c1: Fq::from_str("0").unwrap(),
    };

    let c1 = Fq2 {
        c0: Fq::from_str(
            "1234912246041461878588942434875861039904126177810565185887158306408069993214",
        )
        .unwrap()
            * Fq::from(Fq::R).inverse().unwrap(),
        c1: Fq::from_str(
    ...
    let c3 = Fq2 {
        c0: Fq::from_str(
            "14301738157933195389348253840724448307870907218086206201704502222609770096511",
        )
        .unwrap()
            * Fq::from(Fq::R).inverse().unwrap(),
        c1: Fq::from_str(
            "18766576807588938823931941816656094168356257905513364070341807858523241306211",
        )
        .unwrap()
            * Fq::from(Fq::R).inverse().unwrap(),
    };
    ...
    let c4 = Fq2 {
        c0: Fq::from_str(
            "12945612253170900976712347250337035339258705867784462193943147521219390814770",
        )
        .unwrap()
            * Fq::from(Fq::R).inverse().unwrap(),
        c1: Fq::from_str(
            "21130322481901740787616774064142360811676414460802878397485299194159459008019",
        )
        .unwrap()
            * Fq::from(Fq::R).inverse().unwrap(),
    };
...
pub fn g2Sgn0(u: Fq2) -> u64 {
    ...
    let mut sign_i = *BigUint::to_bytes_le(&t).first().unwrap() as u64 & 1;
    // let mut zero_i = hash2g1::g1NotZero(u.c0);
    let t: BigUint = u.c0.into();
    let zero_i = (*t.to_u64_digits().first().unwrap() == 0) as u64;
    sign |= zero & sign_i;
    zero &= zero_i;

    let t: BigUint = u.c1.into();
    sign_i = *BigUint::to_bytes_le(&t).first().unwrap() as u64 & 1;
...
pub fn g2NotZero(x: Fq2) -> u64 {
    ...
    hash2g1::g1NotZero(x.c0) | hash2g1::g1NotZero(x.c1)
}
...
pub fn HashToG2(msg: &[u8], dst: &[u8]) -> G2Affine {
    ...
    let q0 = MapToCurve2(Fq2 { c0: u[0], c1: u[1] });

    let q1 = MapToCurve2(Fq2 { c0: u[2], c1: u[3] });
...
pub fn EncodeToG2(msg: &[u8], dst: &[u8]) -> G2Affine {
    ...
    let res = MapToCurve2(Fq2 { c0: u[0], c1: u[1] });
...
mod tests {
    ...
    fn MapToCurve2_test() {
        let u = Fq2 {
            c0: Fq::from_str(
                "15963713818282906360305918686195491545577210390832157279818305179904408824931",
            )
            .unwrap(),
            c1: Fq::from_str(
                "2166278439352519416731010325104738631510195416620895094682522641528929475020",
            )
            .unwrap(),
        };

        let q = MapToCurve2(u);
        let expected = G2Affine::new_unchecked(
            Fq2 {
                c0: Fq::from_str(
                    "16872093352184426853297847012752141646605261411290781565485515569233955899058",
                )
                .unwrap(),
                c1: Fq::from_str(
                    "20482288690411193526247554560661659739533735966007371008469181348051437821826",
                )
                .unwrap(),
            },
            Fq2 {
                c0: Fq::from_str(
                    "427035866446275812154335387235552457760650543923113579505536211797911740485",
                )
                .unwrap(),
                c1: Fq::from_str(
                    "14849552243024588631071292176876897701191437999604860450422231174965236442203",
                )
                .unwrap(),
            },
        );
        ...
        let u = Fq2 {
            c0: Fq::from_str(
                "12752967732566665017975022503761080419696068755373050496264700974774108086129",
            )
            .unwrap(),
            c1: Fq::from_str(
                "20655422394809824901799481664662586419100706577355794400212187554951433717414",
            )
            .unwrap(),
        };
        ...
        let expected = G2Affine::new_unchecked(
            Fq2 {
                c0: Fq::from_str(
                    "12193882055337081757241417044229479753659926309860257758224177044622322698984",
                )
                .unwrap(),
                c1: Fq::from_str(
                    "10092155993942609715417531227866448864240630219985669320168414926220064901453",
                )
                .unwrap(),
            },
            Fq2 {
                c0: Fq::from_str(
                    "21850450548984866542151665069165216760882062028063278212318726360439829725223",
                )
                .unwrap(),
                c1: Fq::from_str(
                    "10197523149668572844555341938160230574503097016636734560718180396672437043430",
                )
                .unwrap(),
            },
        );
        ...
        let u = Fq2 {
            c0: Fq::from_str(
                "18898141882839095816276844526801422247849121311000147859768000750276893266433",
            )
            .unwrap(),
            c1: Fq::from_str(
                "3788127287937052767604234353437582991385298973804519256517508390161626404924",
            )
            .unwrap(),
        };
        ...
        let expected = G2Affine::new_unchecked(
            Fq2 {
                c0: Fq::from_str(
                    "452805888478466390914725495219599183584561454657558688011312346353060651482",
                )
                .unwrap(),
                c1: Fq::from_str(
                    "7959928416860499659800248632934402218020177178560427800377197797165640390130",
                )
                .unwrap(),
            },
            Fq2 {
                c0: Fq::from_str(
                    "14268098188884406522254505541441598455366967966015814006726862271011081843493",
                )
                .unwrap(),
                c1: Fq::from_str(
                    "15148517265986515293057552799755027217326970615601185424102524485888012383276",
                )
                .unwrap(),
            },
        );
    ...
    fn HashToCurve2_test() {
        ...
        let expected = G2Affine::new_unchecked(
            Fq2 {
                c0: Fq::from_str(
                    "10305213714312555419584685236164610766057227018997600762219755820581571775698",
                )
                .unwrap(),
                c1: Fq::from_str(
                    "5140998983273781645596043003996621170933075714207210952317183701750931672829",
                )
                .unwrap(),
            },
            Fq2 {
                c0: Fq::from_str(
                    "12782657610222102886506935265351398708799194735435757564502179253917869011884",
                )
                .unwrap(),
                c1: Fq::from_str(
                    "15746452850775091549966312821847336261590899319279618339578671846526379873840",
                )
                .unwrap(),
            },
        );
        ...
        let expected = G2Affine::new_unchecked(
            Fq2 {
                c0: Fq::from_str(
                    "7947280525355502288245767042139433332619084425813891508679326584140902765312",
                )
                .unwrap(),
                c1: Fq::from_str(
                    "10530141512348869141982713319207053343182583313484148698392330696376288318261",
                )
                .unwrap(),
            },
            Fq2 {
                c0: Fq::from_str(
                    "2079515028849057274649333561166551431956364880890028320215862191123161285080",
                )
                .unwrap(),
                c1: Fq::from_str(
                    "20169147323092870078028771345234445157617856249189458168875341276090072581620",
                )
                .unwrap(),
            },
        );
        ...
        let expected = G2Affine::new_unchecked(
            Fq2 {
                c0: Fq::from_str(
                    "9141649584568251133435811655082820452253999683001609355083509727807340928112",
                )
                .unwrap(),
                c1: Fq::from_str(
                    "19241337378620754008094815492162488101811979191715181531381201352430992486769",
                )
                .unwrap(),
            },
            Fq2 {
                c0: Fq::from_str(
                    "18149222514336885092356998491550186845822771992585824025266466238465484336696",
                )
                .unwrap(),
                c1: Fq::from_str(
                    "9129360097802525322055823374454170177267012396640126715240529872313988489338",
                )
                .unwrap(),
            },
        );
        ...
        let expected = G2Affine::new_unchecked(
            Fq2 {
                c0: Fq::from_str(
                    "20353650816686918912609727598093385895712524005202794071238544969713808081729",
                )
                .unwrap(),
                c1: Fq::from_str(
                    "17684256473523682464984867199875609280081365245056171175421469718260504681254",
                )
                .unwrap(),
            },
            Fq2 {
                c0: Fq::from_str(
                    "15896902550098660794387123920782326368527887924690142904247213645779094259076",
                )
                .unwrap(),
                c1: Fq::from_str(
                    "15390867031388969173331373188576779664345770454778413558467452103273727102977",
                )
                .unwrap(),
            },
        );
        ...
        let expected = G2Affine::new_unchecked(
            Fq2 {
                c0: Fq::from_str(
                    "16357539726107897952076989795377840344861047311782727672153303061989952217690",
                )
                .unwrap(),
                c1: Fq::from_str(
                    "10844839375884734385955874223756004111213539742547007380520745461640534925130",
                )
                .unwrap(),
            },
            Fq2 {
                c0: Fq::from_str(
                    "20703414994053186684664027241143511234937261254193650036949701479117819278515",
                )
                .unwrap(),
                c1: Fq::from_str(
                    "11278285373922966720757356129051535273988981659843897570823718288010165493815",
                )
                .unwrap(),
            },
        );
    ...
    fn encode_to_g2_test() {
        ...
        let expected = G2Affine::new_unchecked(
            Fq2 {
                c0: Fq::from_str(
                    "7290337032722028742894312496454770035215478865307401781131202361899492945880",
                )
                .unwrap(),
                c1: Fq::from_str(
                    "18605632812439984129247614998320701910992924251662446522071513278020164236983",
                )
                .unwrap(),
            },
            Fq2 {
                c0: Fq::from_str(
                    "18565926515830203734257806009639634340842708214357641080049757818108383758101",
                )
                .unwrap(),
                c1: Fq::from_str(
                    "21026435153745179081072575128771379049563093023676092614267505429710510687357",
                )
                .unwrap(),
            },
        );
        ...
        let expected = G2Affine::new_unchecked(
            Fq2 {
                c0: Fq::from_str(
                    "2222545202255207121622252341720884612662004487208664408317925491033383016781",
                )
                .unwrap(),
                c1: Fq::from_str(
                    "3167015911722190124689644160541231412539898594125261078778351544051685395067",
                )
                .unwrap(),
            },
            Fq2 {
                c0: Fq::from_str(
                    "20450065928984038040963910334909877834263207751235246619699259708122680403961",
                )
                .unwrap(),
                c1: Fq::from_str(
                    "4743914079645712786687283872900604142971897405422186449887746314931053675188",
                )
                .unwrap(),
            },
        );
        ...
        let expected = G2Affine::new_unchecked(
            Fq2 {
                c0: Fq::from_str(
                    "7148036967840401493869354348463445038937751410382870212181508408551260940454",
                )
                .unwrap(),
                c1: Fq::from_str(
                    "20374759774184409322905764368361574346849498692562411327726753719663647349306",
                )
                .unwrap(),
            },
            Fq2 {
                c0: Fq::from_str(
                    "10483260641720359876745669935893009958901176103433938324656495809668720301952",
                )
                .unwrap(),
                c1: Fq::from_str(
                    "4967329811281913502786824686629199594924414673725274625361393684486574196665",
                )
                .unwrap(),
            },
        );
        ...
        let expected = G2Affine::new_unchecked(
            Fq2 {
                c0: Fq::from_str(
                    "13125957534682971537993382516266248139111688303006779518993454727694490448781",
                )
                .unwrap(),
                c1: Fq::from_str(
                    "17001778660286232066011321802406169530508086950847889879278742058506410997887",
                )
                .unwrap(),
            },
            Fq2 {
                c0: Fq::from_str(
                    "21493575647416678969105094342755094397493417994062452257278523992404807814786",
                )
                .unwrap(),
                c1: Fq::from_str(
                    "17949331872970707337957877677444470806317301863003904868354967532347837920323",
                )
                .unwrap(),
            },
        );
        ...
        let expected = G2Affine::new_unchecked(
            Fq2 {
                c0: Fq::from_str(
                    "549777038834003445090108363667568008543458018879702424802185895608634070183",
                )
                .unwrap(),
                c1: Fq::from_str(
                    "17241878747514914600777537647804966909993790648100169250922892401106734291369",
                )
                .unwrap(),
            },
            Fq2 {
                c0: Fq::from_str(
                    "8654939251469409238390702955149995024930650667376373847357704134187240104302",
                )
                .unwrap(),
                c1: Fq::from_str(
                    "3048816029845140188911702407210291715056845238089676496464503410870814591366",
                )
                .unwrap(),
            },
        );
...
mod tests {
    ...
    fn hash_to_curve_diff_test_g2() {
        ...
        let result_constantine_aff: G2Affine = unsafe {
            ...
            let result_constantine_sys = mem::transmute::<
                MaybeUninit<bn254_snarks_g2_jac>,
                G2Projective,
            >(result_constantine);
