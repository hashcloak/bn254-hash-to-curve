...
pub fn MapToCurve1(u: Fq) -> G1 {
    ...
    .unwrap();
    z *= Fq::from(Fq::R).inverse().unwrap();
    c1 *= Fq::from(Fq::R).inverse().unwrap();
    c2 *= Fq::from(Fq::R).inverse().unwrap();
    c3 *= Fq::from(Fq::R).inverse().unwrap();
    c4 *= Fq::from(Fq::R).inverse().unwrap();
    ...
    gx1 += Fq::from_str("3").unwrap(); //    14. gx1 = gx1 + B
    ...
    gx2 += Fq::from_str("3").unwrap(); //    20. gx2 = gx2 + B
    ...
    let mut gx = x.square(); //    29.  gx = x²
                             //    30.  gx = gx + A
    gx *= x; //    31.  gx = gx * x
    gx += Fq::from_str("3").unwrap(); //    32.  gx = gx + B
...
fn g1Sgn0(x: Fq) -> u64 {
    ...
    *BigUint::to_bytes_le(&t).first().unwrap() as u64 & 1
}
...
pub fn g1NotZero(x: Fq) -> u64 {
    ...
    t_vec[0] | t_vec[1] | t_vec[2] | t_vec[3]
}
    ...
    fn hash_to_curve_diff_test_g1() {
        ...
        let result_constantine_aff: G1 = unsafe {
            ...
            let result_constantine_sys = mem::transmute::<
                MaybeUninit<bn254_snarks_g1_jac>,
                G1Projective,
            >(result_constantine);
